  ...
  <div class="flex items-start gap-8">
    <div class="flex-1">
      <div class="max-h-[700px] overflow-auto">
        <a-table :columns="columns" :data-source="tags" :pagination="false">
    ...
    <div class="flex-[0_0_380px]">
      ...
      <a-form layout="vertical">
        ...
        <a-form-item label="Key Code" class="!mb-0">
          <a-input v-model="formState.alias" placeholder="Key code" disabled />
        </a-form-item>
...
export default {
  ...
  methods: {
    ...
    onConfirmDelete(record) {
      if (!record?.tag_id) return
      this.$store.dispatch('admin/removeTag', record.tag_id).then(() => {
    ...
    update() {
      ...
      this.$store
        .dispatch('admin/updateTag', {
          id: this.formState.id,
          data: { name: this.formState.name },
        })
        .then(() => {
